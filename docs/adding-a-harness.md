...
| `prompt_via_paste` | `True` if the CLI **can't** reliably take the opening prompt on the command line, so it must be pasted into the TUI after boot instead. Default (absent) = pass the prompt on the CLI like `claude`/`codex`. `opencode`: `True` (see the opencode gotcha below). |
| `tui_ready_marker` | Only read when `prompt_via_paste` is `True`: a substring that appears in the pane once the TUI has painted and is accepting input (e.g. a footer hint bar). `deliver_boot_prompt` waits for this — after it's been *absent* at least once — before pasting. `opencode`: `"ctrl+p commands"`. Re-check the string when bumping the CLI version. |
| `leading_slash_opens_palette` | `True` if a pasted message starting with `/` opens the TUI's command palette instead of submitting. When set, `_guard_paste_text` prepends a space so the message sends as literal text (our skills are invoked as `/name`). `opencode`: `True`. |
| `flatten_paste_newlines` | `True` if the TUI drops newlines from a pasted message (joining lines with no separator → run-on text). When set, `_guard_paste_text` flattens `\n` → space so a multi-line message (e.g. an attributed IWCL message) stays readable. Leave unset for paste-aware TUIs (`claude`, `codex`) that keep pasted newlines as literal line breaks. `opencode`: `True`. |
...
- **Skills.** Wire the agent's skill directory to the wolt's `.claude/skills` (codex reads `.agents/skills`; opencode reads `~/.claude/skills` natively, so nothing is needed). In container mode the wrapper lays that bridge; `ensure_platform_skills` lays it too, so a native wolt gets one as well. The platform re-delivers skills every boot.
...
- **Paste, then submit.** The only universal transport for human-attachable sessions is the tmux paste dance: set-buffer + `paste-buffer -p`, then a separate Enter keystroke. `-p` sends a bracketed paste (ESC[200~ ... ESC[201~) to any TUI that enabled bracketed-paste mode, so the TUI knows exactly where the message ends and the Enter that follows is unambiguously a submit. Without it, a TUI has to guess the paste from input timing and can fold the Enter into the paste as a newline, leaving the message unsent in the composer. `paste_settle` adds a pause before that Enter for a TUI that still needs one. Bench the paste-then-submit flow live against the real TUI: this fails silently, never in a unit test.
- **Opening prompt on the CLI vs. paste (`prompt_via_paste`).** `claude` and `codex` accept the opening prompt as a CLI argument and honor `--model` before running it. `opencode`'s interactive TUI does neither reliably: (1) it dispatches the CLI `--prompt` on a different async path than model resolution, so on a cold model cache the first message runs on whatever provider resolved *first* — in woltspace that's the tunnel-scoped `CLOUDFLARE_API_TOKEN` mis-registered as a Workers-AI provider, which then 401s, instead of the pinned model; and (2) a message starting with `/` opens the TUI's command palette and never submits (typed, pasted, or via `--prompt`), which collides with our `/skillname` boot invocation. (`opencode run <msg>` — the *headless* one-shot — has neither problem, but it exits after replying, so it can't back an attachable session.) The fix is generic and flag-gated so it touches nothing else: set `prompt_via_paste: True` + `tui_ready_marker` + `leading_slash_opens_palette`, and `prepare_session_command` stamps the assembled prompt as `pending_boot_prompt` (omitting the CLI prompt) while `run-session.sh` backgrounds `session-reg deliver-prompt` → `deliver_boot_prompt`, which waits for the pane to paint, then pastes (space-guarding a leading slash). If the TUI dies before painting, the stamp is preserved so a `--resume` respawn redelivers it. The wrapper also warms the model catalog cache (`opencode models`) on a wolt's first boot so `--model` resolves from the start. This is an opencode TUI limitation, not a woltspace one — worth an upstream issue on the model-resolution race.
