...
| Entering a session from the tui inside your own tmux used to move your whole client there | The tui was designed to own the terminal; same-server entry did a `switch-client`, and the usual prefix + d then detached the outer client. | The tui renders the wolt in the pane you are in: a nested client over a pty the tui owns, status bar off for that session, `ctrl-]` (or `WOLTSPACE_TUI_DETACH`) comes back. The tui no longer binds keys on your server. | Nothing - the pane is the unit people already think in. |
