...
| A session whose workdir sits outside the data root shows Claude Code's workspace trust prompt | Preparing a claude session pre-accepts the trust dialog in `~/.claude.json`, but only for directories inside the data root — pointing woltspace at a colony is the trust decision, and it does not extend past it. Everything else still asks, and a headless spawn there parks with nobody to answer. | Trust the directory once by hand (open `claude` in it), or keep session workdirs inside the data root. | Nothing — the data root is the boundary on purpose. |
