...
def test_release_notes_cover_python_intermediate_releases(monkeypatch):
...
def test_saved_plan_cannot_update_non_python_components(plan, legacy):
    if legacy:
        plan['schema'] = 1
    else:
        plan['components'].append({'name': '@woltspace/tui', 'current': '0.5.1', 'target': '0.5.2', 'changed': True})
    with pytest.raises(worker.Failure, match='Python-only'):
        worker.validate_versions(plan)
