...
" "$WOLT_NAME" "$WOLT_DIR/.state"
  tmux send-keys -t main "trust-dir '$WOLT_DIR' && export WOLT_SESSION=main && claude --dangerously-skip-permissions /create-wolt" Enter
else
  # TODO: replace with a /wake skill — check for recent sessions, offer resume or fresh start
  tmux send-keys -t main "trust-dir '$WOLT_DIR' && claude --dangerously-skip-permissions \"hey ${WOLT_NAME}\"" Enter
fi
