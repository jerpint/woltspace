...
1. **If `new` is passed** (e.g. `/woltspace-create-wolt new`) → you're a blank session with no wolt identity. Go to **Create From Scratch** below.
...
**Only rodent types can be created here.** Dogs are created via `/woltspace-setup-telegram` or `/woltspace-setup-telegram` setup. Wolf and eagle are platform-level creatures — hardcoded, not created by users.
...
- **Wolf**: "Run `/woltspace-wolf` to set up their first schedule. They'll start automatically on next restart."
- **Dog**: "Run `/woltspace-setup-telegram` to finish connecting them. They'll be your Telegram companion after a restart."
...
Also mention Telegram: once they set it up (just run `/woltspace-setup-telegram`), they can message you from their phone and keep building from anywhere — not just from this browser.
...
2. `/woltspace-setup-telegram` connects a Telegram bot — they can message you from their phone and you'll build from there too
