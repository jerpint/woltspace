...
You were invoked with two arguments: `<mode> <wolt-name>`.
...
**Apps (`wolts/apps/`) are for things you ship.** They have their own server, dependencies, and `woltspace.json` manifest. They can be public. Don't create apps without the user's permission — suggest one when work outgrows a static site (needs a backend, deps, or sharing). Load the woltspace new-app skill to set one up.
