...
def message_session(session_name: str, text: str) -> dict:
    ...
    try:
        ...
        work_dir = shlex.quote(reg_data.get("dir", "")) if reg_data else ""
        trust_prefix = f"trust-dir {work_dir} && " if work_dir else ""
        resume_cmd = f"{trust_prefix}export WOLT_SESSION={shlex.quote(safe)} && claude --dangerously-skip-permissions {resume_flag} {shlex.quote(text)}"
