...
async def _route_to_session(update: Update, session_name: str, wolt: str, text: str, chat_id: int):
    ...
    if result.get("ok"):
        ...
        if result.get("status") == "revived":
            ...
            await update.message.reply_text(f"🪵 sent to {session_name}\n{session_link}")
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if active_session:
        ...
        await update.message.reply_text(f"🪵 session expired — spawning new one for {active_wolt}")
        try:
            ...
            await update.message.reply_text(f"🪵 new session for {active_wolt}\n{session_link}")
            ...
            await update.message.reply_text(f"couldn't start session for {active_wolt}: {e}")
        return
...
async def handle_voice(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if active_wolt and active_session and _is_session_alive(active_session):
        ...
        try:
            ...
            await update.message.reply_text(f"couldn't start session: {e}")
    else:
        # No active wolt — dog handles
...
async def handle_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not is_allowed(update):
        return
    await update.message.reply_text(f"Hey. I'm {_dog_name()}. Talk to me and I'll connect you to a wolt.")
...
async def handle_setwolt(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    _save_chat_state(chat_id, state)
    await update.message.reply_text(f"Set {name} as the active wolt for this chat. Next message will start a session.")
