"""The WOLTSPACE_* env namespace, and the legacy names still honoured for it.

Every unprefixed variable the platform used to read — `WOLTS_DIR`, `WOLT_DIR`,
`WOLT_NAME`, `WOLT_SESSION` — now has a `WOLTSPACE_`-prefixed canonical name.
Nothing reads the old spellings directly any more; they arrive through
`get_env`, which prefers the new name and falls back to the old one, and
`export_both`, which keeps handing children both so existing wolt skills and
tools do not break.

The inventory of every variable, what consumes it, and the sunset plan lives in
`docs/environment.md`.
"""
