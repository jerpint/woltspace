...
def _interpreter(bot_dir: str, isolation: str) -> tuple[str, ...]:
    ...
    if isolation != "host":
        uv = shutil.which("uv")
        if uv and (Path(bot_dir) / "bot" / "pyproject.toml").is_file():
            return (uv, "run", "--project", "bot", "python")
...
class TelegramConnector:
    ...
    def plan(
        ...
        interpreter = _interpreter(bot_dir, layout.isolation)
