...
async def _send_telegram(session: str, message: str, chat_id: str) -> dict:
    """Send a notification via Telegram with session footer."""
...
async def send_notification(session: str, message: str) -> dict:
    ...
    # Respect the session's adapter — route to wherever it came from
    ...
    if get_env("TELEGRAM_BOT_TOKEN") and telegram_chat_id:
